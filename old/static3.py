...
field are initially 0. The converged solution should equal the initial
conditions.
...
0: rho1  # mass density
1: v1x   # x-component of velocity
2: v1y   # y-component of velocity
3: v1z   # z-component of velocity
4: B1y   # y-component of magnetic field
5: B1z   # z-xomponent of magnetic field
...
def create_training_data(nx, nt):
    ...
    xt_in : np.ndarray, shape (nx*(nt - 1)), 2)
        Array of all [x, t] points within boundary.
    xt_bc : np.ndarray, shape (nx, 2)
...
def compute_boundary_conditions(xt):
    ...
    for (i, (x, t)) in enumerate(xt):
        if np.isclose(t, t0):
            bc[i, :] = [rho10, v1x0, v1y0, v1z0, B1y0, B1z0]
        else:
            raise ValueError
    return bc
